...
def variable_description(column: str) -> str:
    descriptions = {
        ...
        "manual_inclusion_flag": "1 if company was manually added to the sample outside the original shortlist files.",
        "manual_inclusion_reason": "Reason recorded for the manual sample addition.",
        ...
        "cbonds_validated_direct_issue_count": "Number of Cbonds bond issues manually confirmed as direct issues of the current company (including auto-confirmed high-confidence direct matches).",
        "cbonds_validated_direct_issue_flag": "1 if the company has at least one manually validated direct bond issue in Cbonds.",
        "cbonds_group_agent_issue_count": "Number of Cbonds bond issues attributed to a group financing agent rather than the current legal entity.",
        "cbonds_group_agent_issue_flag": "1 if at least one Cbonds bond issue was manually classified as group financing agent debt.",
        "cbonds_same_group_other_legal_issue_count": "Number of Cbonds bond issues linked to another legal entity within the same group.",
        "cbonds_same_group_other_legal_issue_flag": "1 if at least one Cbonds bond issue was manually classified as another legal entity within the same group.",
        "cbonds_structured_linked_issue_count": "Number of structured products (for example Sberbank-linked notes) tied to the company but not issued by the company.",
        "cbonds_structured_linked_issue_flag": "1 if at least one structured linked note was detected for the company.",
        "cbonds_reassign_not_in_sample_issue_count": "Number of Cbonds bond issues that appear to belong to a company outside the current shortlist and should be reassigned if that company is later added.",
        "cbonds_reassign_not_in_sample_issue_flag": "1 if at least one Cbonds bond issue was manually marked as belonging to a company outside the current shortlist.",
        "cbonds_unrelated_excluded_issue_count": "Number of captured Cbonds issue cards manually rejected as unrelated to the company.",
        "cbonds_unrelated_excluded_issue_flag": "1 if at least one captured Cbonds issue card was manually rejected as unrelated.",
        "cbonds_any_related_issue_flag": "1 if the company has at least one related-group issue in Cbonds (financing agent, same-group legal entity, or reassignable outside-sample company).",
        "cbonds_any_nonexcluded_issue_flag": "1 if the company has at least one non-excluded Cbonds issue card after manual validation.",
        "cbonds_missing_strong_issue_count": "Number of strong upstream bond candidates that were not found among the captured Cbonds issue cards and should be checked separately.",
